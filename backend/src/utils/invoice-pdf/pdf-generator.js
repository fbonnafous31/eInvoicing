...
function _getPath(filePath) {
  ...
  logger.info('🟡 _getPath input:', filePath);

  if (filePath.startsWith('invoices/') || filePath.startsWith('factur-x/') || filePath.startsWith('pdf-a3/')) {
    logger.info('🟢 _getPath détecté comme relatif B2:', filePath);
    return filePath;
  }

  if (path.isAbsolute(filePath)) {
    logger.info('🟢 _getPath détecté comme absolu local (ne sera pas modifié):', filePath);
    return filePath;
  }

  const resolved = path.resolve(__dirname, '../../uploads', filePath);
  logger.info('🟢 _getPath relatif local résolu:', resolved);
  return resolved;
}

async function embedFacturXInPdf(pdfPath, facturxPath, attachments = [], invoiceId, title) {
  logger.info('🟡 embedFacturXInPdf inputs:', { pdfPath, facturxPath, attachmentsCount: attachments.length });

  // PDF principal
  const mainPdfPath = _getPath(pdfPath);
  logger.info('🟢 mainPdfPath utilisé:', mainPdfPath);
  const pdfBytes = await storageService.get(mainPdfPath);
  const pdfDoc = await PDFDocument.load(pdfBytes);

  // Factur-X XML
  const facturXPath = _getPath(facturxPath);
  logger.info('🟢 facturXPath utilisé:', facturXPath);
  ...
  for (const att of attachments) {
    ...
    logger.info('🟢 attachment path:', attPath);
  ...
  logger.info('🟢 relativeOutputPath pour sauvegarde:', relativeOutputPath);

  // Sauvegarde
  const finalBytes = await pdfDoc.save();
  const savedPath = await storageService.save(finalBytes, relativeOutputPath);

  logger.info(`✅ PDF/A-3 final prêt : ${savedPath}`);
  return savedPath;
}
